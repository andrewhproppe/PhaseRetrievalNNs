...
from QIML.modules.conversions import *
from sklearn.preprocessing import normalize
from scipy import signal
from QIML.modules.PCFS import energy_to_ζ, spec_corr_to_interferogram_FFT, fourier_ζ_to_ps, threeLorentzian_oneFWHM, make_theta_threeLorentzian_oneFWHM, twoLorentzian_oneJ_norm, make_theta_twoLorentzian_oneJ
from QIML.pipeline.transforms import poisson_sample_log
...
class SimulatedPCFS(object):
    ...
    def plot_g2(self, delays, figsize=(4, 4), dpi=150, xlim=[1e5, 1e13], ylim=[0.5, 1.1], fixframe=False, noisy_g2=False, noise_scale=1):
        if noisy_g2:
            seed: int = 10236
        ...
        for i, t in enumerate(delays):
            ...
            if noisy_g2:
                g2_noisy = poisson_sample_log(self.g2[:, idx], rng, self.df)
                plt.plot(self.τ, g2_noisy, color=self.colors[i], alpha=0.5)
