...
#     """
#     Seems to be working but something probably wrong with dimensions; shape of key_rel is same as 2D
#     """
#     def __init__(
#         self,
#         in_channels,
...
#         ), "dv should be divided by Nh. (example: out_channels: 20, dv: 4, Nh: 4)"
#         assert stride in [1, 2], str(stride) + " Up to 2 strides are allowed."
#         # assert all(s in [1, 2] for s in stride), "Up to 2 strides are allowed."
#
...
#         # if self.relative:
#         #     self.key_rel_w = nn.Parameter(
#         #         torch.randn((size - 1, dk // Nh), requires_grad=True)
#         #     )
#         #     self.key_rel_h = nn.Parameter(
#         #         torch.randn((size - 1, dk // Nh), requires_grad=True)
#         #     )
...
#         # batch, _, depth, height, width = x.size()
#
#         # conv_out
...
#
#         # attn_out
#         # (batch, Nh, depth * height * width, dvh)
#         attn_out = torch.matmul(weights, flat_v.transpose(2, 3))
#         attn_out = torch.reshape(
...
#         q = q.transpose(2, 5).transpose(2, 4).transpose(2, 3)
#
#         rel_logits_w = self.relative_logits_1d(
#             q, self.key_rel_w, D, H, W, Nh, "w"
#         )  # width
#
#         rel_logits_h = self.relative_logits_1d(
#             q.transpose(2, 3), self.key_rel_h, D, W, H, Nh, "h"
#         )  # height
#
#         rel_logits_d = self.relative_logits_1d(
#             q.transpose(2, 4), self.key_rel_d, H, W, D, Nh, "d"
#         )  # depth
#
#         return rel_logits_d, rel_logits_h, rel_logits_w
#
#     def relative_logits_1d(self, q, rel_k, D, H, W, Nh, case):
#         print(case)
#         # rel_logits = torch.einsum("bhxyd,md->bhxym", q, rel_k)
#         rel_logits = torch.einsum("bhzxyd,md->bhzxym", q, rel_k)
#         rel_logits = torch.reshape(rel_logits, (-1, Nh * D * H, W, 2 * W - 1))
#         rel_logits = self.rel_to_abs(rel_logits)
#
#         rel_logits = torch.reshape(rel_logits, (-1, Nh, D, H, W, W))
#         rel_logits = torch.unsqueeze(rel_logits, dim=4)
#         rel_logits = rel_logits.repeat((1, 1, 1, 1, D, W, 1))
#
#         if case == "w":
...
#             )
#         rel_logits = torch.reshape(rel_logits, (-1, Nh, D * H * W, D * H * W))
#         return rel_logits
#
#     def rel_to_abs(self, x):
#         B, Nh, L, _ = x.size()
...
class AugmentedConv3D(nn.Module):
    ...
    def __init__(
        ...
        if self.relative:
            self.key_rel_w = nn.Parameter(
                torch.randn((3 * self.shape - 1, dk // Nh), requires_grad=True)
            )
            self.key_rel_h = nn.Parameter(
                torch.randn((3 * self.shape - 1, dk // Nh), requires_grad=True)
            )
    ...
    def relative_logits_1d(self, q, rel_k, D, H, W, Nh, case):
        rel_logits = torch.einsum("bhzxyd,md->bhzxym", q, rel_k)
        # Nh * H * W * (H * W * D - 1)
        rel_logits = torch.reshape(rel_logits, (-1, Nh * H, W, (D + H + W) - 1))
        rel_logits = self.rel_to_abs(rel_logits)

        rel_logits = torch.reshape(rel_logits, (-1, Nh, D, H, W, W))
        rel_logits = torch.unsqueeze(rel_logits, dim=4)
        rel_logits = rel_logits.repeat((1, 1, 1, 1, H, 1, 1))
...
if __name__ == "__main__":
    ...
    # stride = (2, 2, 2)
    stride = 2
    # shape = (nframe // stride[0], npix // stride[1], npix // stride[2])
    ...
    tmp2 = torch.randn((2, in_channels, nframe, npix, npix)).to(device)
